...
def accuracy_teams(X, y, df):
    ...
    Clust_train = Xtrain['Clust']
    Clust_test = Xtest['Clust']
    Xtest.drop(columns=['Team', 'Clust'], inplace = True)
    Xtrain.drop(columns=['Team', 'Clust'], inplace = True)
    ...
    df_preds['Clust']= Clust_test
    
    lista_equipos = df['Home'].unique()
    d = {}
    for i in lista_equipos:
        df_equipo = df_preds[df_preds['Home']==i]
        d[i]=[round(accuracy_score(df_equipo['Result'], df_equipo['Predict']),2), list(df_equipo['Clust'].unique())[0]]
    accuracy_teams = pd.DataFrame(d).T
    accuracy_teams.rename(columns={0:'Accuracy', 1:'Cluster'}, inplace=True)

    return df_preds, accuracy_teams.reset_index()

def show_accuracy_teams(df):
    sns.barplot(data=df, x='index', y='Accuracy')
    plt.xticks(rotation=90)
    plt.tight_layout()
    plt.show()
    
def show_accuracy_clust(df):
    df_clust=df[['Accuracy', 'Cluster']].groupby('Cluster').mean().reset_index()
    df_clust
    
    sns.barplot(data=df_clust, x='Cluster', y='Accuracy')
    plt.tight_layout()
    plt.show()
